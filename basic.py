def add_number (num1, num2):
    return num1 + num2
# here i used int to convert a string "input" into an integer so that the result wont concatinate, this will prevent you from getting concatinated values intead of the sum. We can use float instead of int to handle decimal places 
num1 = float(input("Enter the first number: "))
num2 = float(input("Enter the second number: "))

# Here i called the add_number function
...
# reversing the text using slicing notation. These means that take the string from start to end in reverse order 
    return text [:: -1]
...
# then i will initialize the count for the vowels
    count = 0

    # go through each character in the input text
    for nature in text:

        # here i am checking if the chacter is a vowel
        if nature in vowels:
            count += 1

            # then return the total count of vowels
    return count 
...
def get_persons():
    ...
    while True :
        ...
        # Then add name and age as a tuple to the list
...
def merge_dicts (dict1, dict2):
    ...
    for key, value in dict2.items():
        if key in merged_dict:
            #This Sum the values if the key already exists
